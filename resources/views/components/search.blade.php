...
<div {{ $attributes->merge(['class' => "md:flex items-center md:space-x-2 space-y-2 md:space-y-0 w-full max-w-2xl m-auto"]) }}>
    <form action="{{ route('announcement.search', ['category' => $category?->slug]) }}" class="rounded-full flex border-2 border-indigo-600 bg-white p-1 md:p-2 w-full items-center">
        ...
        <button @click="sidebarOpen = true" type="button"
            @class(['text-gray-900 hover:text-indigo-700 text-xl lg:hidden p-1 md:p-2', 'hidden' => !$filterButton])
        >
