...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-roboto bg-gray-50 min-h-dvh flex flex-col flex-1" x-data="{ sidebarOpen: false }" :class="sidebarOpen ? 'overflow-hidden' : ''">
        ...
        @if (isset($headerNavigation))
            <div class="w-full hidden lg:block fixed top-0 h-10 bg-black z-40">
                {{ $headerNavigation }}
            </div>
        @endif

        <div @class(['flex w-full flex-1 flex-col', 'pt-0 lg:pt-10' => isset($headerNavigation), 'pb-10 lg:pb-0' => isset($footerNavigation)])>
            @if (isset($sidebar))
            ...
            <div @class(['w-full h-full flex-1 flex flex-col', 'lg:pl-72 xl:pl-96' => isset($sidebar)])>
                @if (isset($header))
                    <div class="flex w-full px-2 min-h-10 items-center py-1 space-x-2 bg-gray-50 z-30 sticky lg:top-10 top-0" x-data="{ headerOpen: false }">
