<x-layout x-data="">
    ...
    <div class="lg:flex w-full max-w-6xl m-auto lg:py-12 lg:space-x-3 lg:px-3 space-x-0 space-y-6 lg:space-y-0">
        <div class="w-full lg:w-2/3 space-y-3 lg:space-y-6">
            ...
            <div class="space-y-6 p-2 md:p-6 bg-white lg:rounded-xl shadow-xl">
                <div class="space-y-2">
                    <h2 class="font-bold text-3xl">
                        {{ ucfirst($announcement->title) }} m<sup>2</sup>
                    </h2>
                    <p class="text-gray-600 text-xl">
                        {{ $announcement->address }}, {{ $announcement->location['name'] }}
                    </p>
                    <p class="font-bold text-indigo-600 text-2xl">
                        {{ $announcement->price }} {{ $announcement->price_currency ?? "Kc" }} 
                    </p>
                ...
                <div class="w-full space-y-1">
                    @if ($announcement->price)
                        <div class="flex justify-between">
                            <h4 class="text-gray-500">{{ __("Price:") }}</h4>
                            <p class="text-indigo-700">{{ __($announcement->price) }} {{ $announcement->price_currency }}</p>
                        </div>
                        <hr class="border-dashed">
                    @endif

                    @if ($announcement->utilities)
                        <div class="flex justify-between">
                            <h4 class="text-gray-500">{{ __("Utilities:") }}</h4>
                            <p>{{ __($announcement->utilities) }} {{ $announcement->utilities_currency }}</p>
                        </div>
                        <hr class="border-dashed">
                    @endif
                    ...
                    @if ($announcement->deposit)
                        <div class="flex justify-between">
                            <h4 class="text-gray-500">{{ __("Deposit:") }}</h4>
                            <p>{{ __($announcement->deposit) }} {{ $announcement->deposit_currency }}</p>
                        </div>
                        <hr class="border-dashed">
                    @endif
                    ...
                    @if ($announcement->condition)
                        <div class="flex justify-between">
                            ...
                            <p>{{ ucfirst(__($announcement->condition->name)) }}</p>
                        </div>
                        <hr class="border-dashed">
                    @endif
                    ...
                    @if ($announcement->floor)
                        <div class="flex justify-between">
                            <h4 class="text-gray-500">{{ __("Floor:") }}</h4>
                            <p>{{ $announcement->floor }}</p>
                        </div>
                        <hr class="border-dashed">
                    @endif

                    @if ($announcement->square_meters)
                        <div class="flex justify-between">
                            <h4 class="text-gray-500">{{ __("Square meters:") }}</h4>
                            <p>{{ __($announcement->square_meters) }} m<sup>2</sup></p>
                        </div>
                        <hr class="border-dashed">
                    @endif

                    @if ($announcement->equipment)
                        <div class="flex justify-between">
                            <h4 class="text-gray-500">{{ __("Equipment:") }}</h4>
                            <p>{{ ucfirst(__($announcement->equipment->name)) }}</p>
                        </div>
                        <hr class="border-dashed">
                    @endif

                    @if ($announcement->additional_spaces)
                        <div class="flex justify-between">
                            <h4 class="text-gray-500">{{ __("Additionla spaces:") }}</h4>
                            <div class="text-right">
                                @foreach ($announcement->additional_spaces as $additional_space)
                                    <x-badge>{{ __($additional_space->name) }}</x-badge>
                                @endforeach
                            </div>
                        </div>
                    @endif
